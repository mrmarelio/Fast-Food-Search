...
<nav class="navbar">
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
            <h2 style ="text-align:center;">Kokios prekės ieškote ?</h2>
            <form style ="text-align:center;" action="ieskotiPrekes.php" method="POST">
                <input type="text" name="ieskoti" placeholder="Įveskite prekės pavadinimą">
                <button type ="submit" name ="ieskotiPrekes">Ieškoti prekės</button>
            </form>
            </div>
        </div>
  
    <?php
        if(isset($_POST['ieskotiPrekes'])){
            $search = mysqli_real_escape_string($conn, $_POST['ieskoti']);
            $sql = "SELECT * FROM produktas WHERE Product_Name LIKE '%$search%'";
            $result = mysqli_query($conn, $sql);
            $queryResult = mysqli_num_rows($result);

            if($queryResult > 0 and $search != ''){
